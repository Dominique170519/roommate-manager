...
- **值日偏好**：每位室友可标「首选区域」（优先排给你）与「不方便区域」（自动跳过），排班在公平轮换基础上尽量满足个人意愿，本周与下周排班实时生效、带「首选」徽标
- **历史补偿**：首选撞车或区域没人愿意时自动记补偿分——让位 +1、捡到不方便区域 +2；下次排班补偿分高的人优先满足首选、捡脏活优先给低分者，长期谁都不吃亏（带「补偿 +N」徽标）
...
| 值日 · 排班打卡与偏好设置 | 值日偏好 · 首选 / 不方便区域 |
|---|---|
| ![值日](assets/shot-cleaning.png) | ![偏好](assets/shot-prefs.png) |

| 物品 · 采购清单与登记 | 公约 · 签署与编辑 |
|---|---|
| ![物品](assets/shot-items.png) | ![公约](assets/shot-rules.png) |

## 技术实现

- **单文件应用**：一个 `index.html` 内含全部 HTML / CSS / JavaScript，零依赖、零构建，拿到就能用
- **原生技术栈**：原生 JS + Hash 路由 + `localStorage` 持久化（无框架、无后端）
- **排班算法**：按周轮换 + 偏好感知（首选优先、不方便跳过）+ 历史补偿（让位/捡脏活记分、高分者优先），无偏好无积分时与纯轮换输出完全一致（已回归验证）
